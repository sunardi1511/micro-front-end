...
const Courses = ({ data }) => {
    ...
    const handleSearch = (event) => {
        ...
        timeoutSearch.current = setTimeout(() => {
            ...
            courses.all({ params: { q: event.target.value } })
                .then((res) => {
                    setSearchResponse({
                        isLoading: false,
                        isError: false,
    ...
    return (
        ...
        <>
            ...
            <section className='pt-10 z-30 relative' style={{ height: 360 }}>
                ...
                <div className='meta-title absolute bottom-0 object-fill z-0 w-full flex justify-center items-center'
                    ...
                    <div>
                        ...
                        <h4 className='text-lg text-center text-white'>
                            Jangan mau kalah update dengan lainnya.
                            <br />
                            Yuk ikut perkembangan teknologi
                        </h4>
                        <div className='flex flex-col relative' ref={selectWrapper}>
                            <input
                                ...
                                value={Search}
                                placeholder={SearchFocus ? "Ketik minimal 3 kerakter untuk mencari"
                                    : "Lagi nyari kelas apa?"}
                                className='bg-white focus: outline-none transition-all duration-200
                                 focus:border-teal-500 border-gray-600 px-4 py-3 w-full mt-6'/>
                            {
                                Search.length >= 3 && (
                                    <div
                                        ...
                                        style={{ top: 75 }}>
                                        {
                                            SearchResponse.isLoading ? ("Loading..") :
                                                <>
                                                    {
                                                        SearchResponse.isError && "Someting is technically wrong"
                                                    }
                                                    {
                                                        ...
                                                        SearchResponse?.data?.map((item, index) => {
                                                            return (
                                                             ...
                                                             cursor-pointer hover:bg-gray-200 relative'>
                                                                    ...
                                                                    <div className='w-full px-4'>
                                                                        ...
                                                                        <p className='text-gray-600'>{item?.level ?? "Lavel"}</p>
                                                                        <Link href='/courses/[id]' as={`/courses/${item.id}`}><a className='link-wrapped'>
                                                                        </a></Link>
                                                                    </div>
                                                                </div>
                                                            )
